...
features7 = unique(unlist(lapply(Bimod.res, function(x) names(x)[1:n])))
features8 = unique(unlist(lapply(SCVI.res, function(x) names(x)[1:n])))
features9 = unique(unlist(lapply(M3DROP.res, function(x) names(x)[1:n])))
...
## Visualize trajectories
```{r fig.height=20, fig.width=10}
patchwork::wrap_plots(list_full_model$celltype_plot[c(2:10, 1)], 
                      ncol = 4, nrow = 3) %>%
  ggsave(filename="../../../Figures/trajectory_analysis/trajectory_dx_trajectories_mst_additional_methods.pdf", width=20, height=10)
```
